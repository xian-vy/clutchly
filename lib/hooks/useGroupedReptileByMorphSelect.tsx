...
export function useGroupedReptileByMorphSelect({ filteredReptiles,disabled }: Props ) {
  ...
  const groupedReptiles = useMemo(() => {
    return morphs.map(morphItem => ({
      label: morphItem.name,
      items: filteredReptiles
        .filter(reptile => reptile.morph_id.toString() === morphItem.id.toString())
        .map(reptile => ({
          value: reptile.id,
          label: reptile.name,
