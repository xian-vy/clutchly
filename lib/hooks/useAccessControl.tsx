...
const useAccessControl = (user: User | undefined): UseAccessControlReturn => {
  ...
  const accessProfile = useMemo(() => {
    ...
    return (
      accessProfiles.find((profile) => profile.id === user.access_profile_id) ||
      null
    );
  }, [user, accessProfiles]);
  ...
  const hasAccess = useCallback(
    (
      ...
      const accessControl = accessProfile.access_controls.find(
        (ac) => ac.page_id === pageId
      );
    ...
    (items: NavItem[]): NavItem[] => {
      ...
      return items.filter((item) => {
        ...
        if (item.name.toLowerCase() === "settings")
          return user && (user.role === "admin" || user.id === user.org_id);
        // Special case: Only org owners can see Users page
        if (item.name.toLowerCase() === "users")
          return user && user.id === user.org_id;

        // Find the page corresponding to the current navigation item
        const itemPage = pages.find(
          (p) =>
        ...
        if (item.items) {
          ...
          return (
            (itemPage && hasAccess(itemPage.id, "view")) ||
