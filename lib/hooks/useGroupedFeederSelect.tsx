...
export function useGroupedFeederSelect() {
  ...
  const groupedFeeders = React.useMemo<GroupedFeeder[]>(() => {
    return feederTypes
      .map((types) => ({
        ...
        items: feederSizes
          .filter(
            (sizes) => sizes.feeder_type_id.toString() === types.id.toString()
          )
          .map((sizes) => ({
            value: sizes.id,
            label: sizes.name,
  ...
  const FeederSelect = React.useMemo(() => {
    return React.memo<FeederSelectProps>(function FeederSelect({
      value,
      onValueChange,
        ...
        () =>
          value
            ? feederTypes.find(
                (types) =>
                  types.id.toString() ===
                  feederSizes.find((sizes) => sizes.id.toString() === value)
                    ?.feeder_type_id
              )?.name
            : null,
